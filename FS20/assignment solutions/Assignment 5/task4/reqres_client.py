...
def query_user_list(url):
    ...
    if response.status_code == 200:
        ...
        # extract data received from API which is interesting
        users = response.json()['data']
        
        # create for each usera python User object
        for x in users:
            
            # create new python object of class User
            u = User(x["first_name"], x["last_name"], x["email"], x["id"])
...
def create_new_user(url, user):
    ...
    # create user by executing a post request
    
    response = requests.post(url, data = {"first_name" : user.first_name, "last_name" : user.last_name, "email" : user.email})
    
    # check if user creation was succesful (HTTP code 201 means "Created")
    if response.status_code == 201:
        
        # get user id and convert it to integer
        ...
        # return None as no user has been created
        return (None, None)
