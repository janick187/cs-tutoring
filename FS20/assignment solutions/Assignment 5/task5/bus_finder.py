...
def get_location_name(location_id):
    ...
    # execute the get request and extract the station name out of the json response
    station_name = requests.get(url, params=params).json()['stations'][0]['name']
...
def find_next_bus(origin, destination, time, limit=10):
    '''
    Send an HTTP GET request to http://transport.opendata.ch/v1 API
    ...
    # format the departure date and time provided so that it can be used for the request
    date = time.strftime("%Y-%m-%d")
    time = time.strftime("%H:%M")
    
    # create a dictionary with all required parameters with all parameters
    params = {
        'from' : origin,
        'to' : destination,
        'date' : date,
    ...
    # get all the connections received
    next_buses = requests.get(url, params=params).json()['connections']
    
    # iterate over each connection
    for bus in next_buses:
        # extract data out of json string
        departure_time = datetime.fromtimestamp(bus['from']['departureTimestamp'])
        arrival_time = datetime.fromtimestamp(bus["to"]["arrivalTimestamp"])
        operator = bus["sections"][0]["journey"]["operator"]
        number = bus["sections"][0]["journey"]["number"]
        
        # creat new bus object
        bus = Bus(operator, number, departure_time, arrival_time)
